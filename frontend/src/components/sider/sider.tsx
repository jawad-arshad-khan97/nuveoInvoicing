...
export const ThemedSiderV2: React.FC<RefineThemedLayoutV2SiderProps> = ({
  ...
  const groupMenuItems = (menuItems: ITreeMenu[]) => {
    // Example: Grouping based on a meta property
    const grouped: Record<string, ITreeMenu[]> = menuItems.reduce(
      (acc, item) => {
        const group = item.meta?.group || "default";
        if (!acc[group]) acc[group] = [];
        acc[group].push(item);
        return acc;
      },
      {} as Record<string, ITreeMenu[]>
    );
  ...
  const renderGroupedMenu = (menuItems: ITreeMenu[], selectedKey?: string) => {
    const groupedMenu = groupMenuItems(menuItems);

    return groupedMenu.map(({ groupName, items }) => (
      <Menu.ItemGroup key={groupName} title={groupName}>
        {renderTreeView(items, selectedKey)}
      </Menu.ItemGroup>
    ));
  };
